...
/**
 * 格式化app json数据为亚马逊商品上传json数据(该方法使用在商品下架和重新上架时使用不需要ean的时候)
 *
 * @author EDZ
 */
@Configuration
@AllArgsConstructor
public class FormateAmazonNotEanJsonUtil {
    ...
    public JSONArray formateJonsForAmazon(List<GoodsProduct> goodsProducts,String marketPlaceId,String shopId) {
        ...
        for (int h = 0; h < goodsProducts.size(); h++) {
            ...
            if (length < 1900) {
                obj.put("description", HtmlRepliceUtils.parseAllHtmlInfo(goodsProduct.getDescription()));
            } else {
                obj.put("description", HtmlRepliceUtils.parseAllHtmlInfo(goodsProduct.getDescription().substring(0, 1900)));
            }
            //产品品牌
            if (StringUtils.isNoneBlank(goodsProduct.getBrandName())) {
                obj.put("brand", goodsProduct.getBrandName());
            } else {
                obj.put("brand", "168pro"+(int)((Math.random()*9+1)*1000000));
            }
            //亚马逊上传产品后的亮点
            ...
            if (goodsProduct.getProductVariants().size()>0) {
                ...
                if (StringUtils.isNoneBlank(variantColor) && StringUtils.isNoneBlank(variantSize)) {
                    ...
                    for (int j = 0; j < goodsProduct.getProductVariants().size(); j++) {
                        ...
                        if(productVariant.getImageList()!=null && productVariant.getImageList().size()>0){
                ...
                //图片信息
                JSONArray imagesArray = new JSONArray();
                if(goodsProduct.getImageList()!=null && goodsProduct.getImageList().size()>0){
